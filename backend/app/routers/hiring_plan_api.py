...
def create_row(body: HPRowIn, user: dict = Depends(get_current_user)):
    ...
    query(
        ...
        [
            ...
            body.headcount or 1, body.priority, body.band, body.experience,
...
def create_req_from_plan(row_id: str, user: dict = Depends(get_current_user)):
    ...
    # Auto-assign recruiter if current user is one
    if user["role"] == "recruiter":
        query(
            "INSERT INTO requisition_recruiter (requisition_id, recruiter_id) VALUES (%s,%s) ON CONFLICT DO NOTHING",
            [req_id, user["sub"]], fetch=False,
        )
...
def export_excel(
    ...
    ws = wb.active
    ws.title = "Hiring Plan"

    hdr_fill = xs.PatternFill("solid", fgColor="0C0D10")
    hdr_font = xs.Font(bold=True, color="FFFFFF")

    for ci, (label, _) in enumerate(_EXCEL_COLS, 1):
        c = ws.cell(1, ci, label)
        c.fill = hdr_fill
        c.font = hdr_font

    for ri, row in enumerate(computed, 2):
        for ci, (_, field) in enumerate(_EXCEL_COLS, 1):
            val = row.get(field)
            if isinstance(val, (date, datetime)):
                ws.cell(ri, ci, val)
            elif isinstance(val, float) and val == int(val):
                ws.cell(ri, ci, int(val))
            else:
                ws.cell(ri, ci, val)
    ...
    wb.save(buf)
    buf.seek(0)
    fname = f"egnex_hiring_plan{'_'+fy if fy else ''}.xlsx"

...
def list_sendable(user=Depends(_require_send_access)):
    """Return custom templates available for the manual send dialog."""
...
def send_email_to_candidate(app_id: str, payload: SendEmailPayload, user=Depends(_require_send_access)):
    ...
    Fills {{candidate_name}}, {{job_title}}, {{company_name}}, {{recruiter_name}}
    from the application record. Blocks if any placeholder cannot be filled.
    Logs the send to sent_email_log.
    """
    key = payload.template_key

    if key in BUILTIN_KEYS:
        raise HTTPException(400, "Built-in templates cannot be used for manual send. Create a custom template.")
    ...
    app_row = query_one(
        """SELECT
           ...
           LEFT JOIN app_user u ON u.id = a.recruiter_id
           WHERE a.id = %s""",
        [app_id],
    )
    if not app_row:
    ...
    if missing:
        raise HTTPException(
            ...
            "Update the template to use only: candidate_name, job_title, company_name, recruiter_name."
        )
    ...
    # Send
    try:
        send_email(candidate_email, rendered_subject, rendered_body)
    except Exception as exc:
        raise HTTPException(500, f"Email delivery failed: {exc}")
    ...
    try:
        query(
            ...
            [app_id, key, tmpl_row["name"], candidate_email, user["sub"],
             rendered_subject, payload.notes or None],
            fetch=False,
        )
    except Exception as exc:
